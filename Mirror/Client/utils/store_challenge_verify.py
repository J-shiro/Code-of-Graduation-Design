...
class SCV:
    ...
    def Send_Challenge_Receive_Response_Verify(self, tuples, R):
        # 套接字连接服务器
        client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        server_address = ('localhost', 15323)
        client_socket.connect(server_address)
        ...
        try:
            ...
            for item in tuples:
                ...
                for j in range(self.s):
                    for lil_R in R:
                        temp_multi *= ga_3d_matrix[lil_R-1][c_index][j]*hb_3d_matrix[lil_R-1][c_index][j]
            ...
            for j in range(self.s):
                ...
                left_result = gmpy2.mul(left_result, gmpy2.powmod(mu[j], self.epsilon[j]+len(R), self.N))
            left_result = left_result % self.N
