...
class RRR:
    ...
    def Receive_Tag_and_File(self, connection):
        ...
        # 接收sk, u
        sk_size_data = connection.recv(4)
        sk_size = struct.unpack('!I', sk_size_data)[0]

        sk = b''
        while len(sk) < sk_size:
            sk += connection.recv(sk_size - len(sk))

        self.sk = sk

        u_size_data = connection.recv(4)
        u_size = struct.unpack('!I', u_size_data)[0]

        u = b''
        while len(u) < u_size:
            u += connection.recv(u_size - len(u))
        u = int(u)
        ...
        return D_file, sigma_list, ga_final, hb_final, alpha_prime, beta_prime, sk, u
    ...
    def Receive_Challenge(self, connection):
        ...
        # 解包数据
        unpacked_data = struct.unpack('i' + 'B'*(len(recv_data)-4), recv_data)

        S_length = unpacked_data[0]
        S = list(unpacked_data[1:])
        self.S = S
    ...
    def server_Response(self):
        ...
        for item in self.tuples:
            ...
            for j in self.S:
                for lil_R in self.R:
                    temp_multi *= r_D_file[lil_R-1][c_index][j-1]
    ...
    # 接收对第r个副本的第n个块的s-1个扇区的验证参数
    def Receive_s_verify(self, connection):
        ...
        # 解包数据
        unpacked_data = struct.unpack('iii' + 'B'*(len(recv_data)-12), recv_data)

        # 分别获取解包后的r、n和v_s
        r, n, v_s_length = unpacked_data[:3]
        v_s = list(unpacked_data[3:])

        return r, n, v_s

    # 打包返回s-1个扇区值
    def Send_back_sectors(self, r, n, v_s, connection):
        # 路径
        folder = 'file_folder'
        replica_name = 'r_file.txt'
        ...
        # 对应r, n的sectors
        temp_s = r_D_file[r-1][n-1]

        response_s = []
        for i in v_s:
            response_s.append(temp_s[i-1])

        # 打包response_s 发送回用户
        s_length = len(response_s)
        packed_data = struct.pack("!I", s_length)
        for i in range(s_length):
            s_str = str(response_s[i])
            s_bytes = s_str.encode()
            packed_data += struct.pack("!I", len(s_bytes))
            packed_data += s_bytes
