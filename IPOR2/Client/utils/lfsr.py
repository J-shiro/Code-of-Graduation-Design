...
class LFSR:
    ...
    # 使用LFSR生成标签
    def Process_LFSR(self, nambda, final_stat_length, ord, generator,c, c_prime, N):
        '''
            会生成后续需要的g^a, h^b(public), 以及a, b(secret)
        '''
        ini_nambda_stat = self.Generate_ini_state(nambda, ord)

        final_stat_secret = self.secret_LSFR(ini_nambda_stat, c, final_stat_length, N)

        final_stat_public = self.public_LSFR(final_stat_secret[0:2*nambda], c_prime, final_stat_length, N, generator)

        return ini_nambda_stat, final_stat_secret, final_stat_public
    
    # 使用LFSR生成初始的a和b的列，以及初始的ga和hb的列
    def Process_LFSR_ini(self, nambda, ord, generator, c, c_prime, N):
        
        ini_nambda_stat = self.Generate_ini_state(nambda, ord)

        # 生成两倍nambda的后续序列
        ini_2nambda_stat = self.secret_LSFR(ini_nambda_stat, c, 2*nambda, N)

        # 通过两倍nambda的序列生成初始的 2*nambda 长度的 ga 的初始序列
        leng = len(ini_2nambda_stat)

        ini_ga = [gmpy2.powmod(generator, x, ord) for x in ini_2nambda_stat]

        return ini_nambda_stat, ini_2nambda_stat, ini_ga
